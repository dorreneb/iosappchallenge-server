...
(defn uuid
  ...
  ([x] (if (string? x) (java.util.UUID/fromString x) x)))
...
(defn rename-box [graph {:keys [session-id id body] :as message} me]
  (dosync
   (let [n (find-element session-id (:id body))]
...
(defn delete-box [graph {:keys [session-id id] :as message} me]
  (dosync
   (let [element (element-by-uuid session-id id)]
     (delete-connectors-for-box session-id id)
     (send graph (fn [state] (filter (partial not= element) state)))
...
(defmethod update-graph "rename-box" [message requester]
  (rename-box (graph-agent (:session-id message)) message requester))

(defmethod update-graph "move-box" [message requester]
  (move-box message requester))

(defmethod update-graph "delete-box" [message requester]
  (delete-box (graph-agent (:session-id message)) message requester))
